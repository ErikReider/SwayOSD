...
impl SwayosdWindow {
    ...
    pub fn new(app: &gtk::Application, monitor: &gdk::Monitor) -> Self {
        ...
        Self {
            ...
            slots: Arc::new(Mutex::new(ContentSlots::default())),
        }
    }
    ...
    pub fn changed_brightness(&self, brightness_backend: &mut dyn BrightnessBackend) {
...

        {
            ...
            if get_show_percentage() {
                slots.percentage_text = Some(format!("{}%", (brightness / max * 100.).round() as i32));
                slots.percentage_min_chars = Some(4);
            }
        }
    ...
    pub fn changed_keylock(&self, key: KeysLocks, state: bool) {
        ...
        let (label_text, symbol) = match key {
            KeysLocks::CapsLock => {
                (format!("Caps Lock {}", on_off_text), "caps-lock-symbolic")
            }
            KeysLocks::NumLock => {
                (format!("Num Lock {}", on_off_text), "num-lock-symbolic")
            }
            KeysLocks::ScrollLock => {
                (format!("Scroll Lock {}", on_off_text), "scroll-lock-symbolic")
            }
        };
    ...
    fn run_timeout(&self) {
        ...
        if slots.label_text.is_some() || slots.progress_type.is_some() {
            ...
            if let Some(progress_type) = &slots.progress_type {
                let progress: gtk::Widget = match progress_type {
                    ProgressType::Normal(fraction) => self.build_progress_widget(*fraction).upcast(),
                    ProgressType::Segmented(value, n_segments) => {
                        self.build_segmented_progress_widget(*value, *n_segments).upcast()
                    }
                };
        ...
        let percentage_widget = slots.percentage_text.as_ref().map(|text| {
            self.build_text_widget(Some(text), slots.percentage_min_chars)
        });
        ...
        if slots.apply_icon_margin_to_label {
            if let (Some(icon), Some(label)) = (icon_widget.as_ref(), label_widget.as_ref()) {
